...
function ForecastCard({ runId }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      axios.get(`${API_URL}/cash-forecast/timeseries/${runId}`),
    ])
      .then(([snapshotResponse, seriesResponse]) => { if (active) setForecast({ ...snapshotResponse.data, timeseries: seriesResponse.data }) })
  ...
  const series = forecast.timeseries?.forecast ?? []
  ...
  return <section className="forecast-card" aria-label="Cash position">
    ...
    <div className="forecast-series"><p className="eyebrow">Projected cash</p><div className="table-wrap"><table><thead><tr><th>Period</th><th>Expected inflow</th><th>Expected outflow</th><th>Projected cash</th></tr></thead><tbody>{series.map((item) => <tr key={item.period}><td>{item.period}</td><td>{formatAmount(item.expected_inflow_paise)}</td><td>{formatAmount(item.expected_outflow_paise)}</td><td><strong>{formatAmount(item.projected_cash_paise)}</strong></td></tr>)}</tbody></table></div></div>
